...
function Visualizza(props) {
  const email = "peppe@gmail.com";
    const { task_title, id, fetchTaskToDo} = props;
  ...
  const fetchModify = async () => {
    console.log(modify);
    const response = await axios.put(`http://192.168.1.11:3000/task/modificatask`, modify);
  ...
  const Delete = async () => {
    ...
    const response = await axios.delete(`http://192.168.1.11:3000/task/${modify.id}`);
    const results = response.status;
    if (results == 200) {
        console.log(response);}
        fetchTaskToDo()
  ...
  const changestate = async () => {
    const response = await axios.put(`http://192.168.1.11:3000/task/cambiastato`, {id ,email,'state':'working at'});
    const results = response.status;
    if (results == 200) {
        console.log(response);}
        fetchTaskToDo()
        ...
        <div className="modal-overlay">
          <div className="modal_alfonso">
            <div className="modal-content_alfonso">
              ...
              <textarea id="desc" className="description_alfonso" onChange={e => {setDescription(e.target.value)}} placeholder="Task bella"></textarea>
              <div className="position-button_alfonso">
                ...
                <button className='button-cancel_alfonso'onClick={closeModal}>Cancel</button>
                  <button className='button-add_alfonso' type="submit" onClick={async (e)=>{e.preventDefault()
